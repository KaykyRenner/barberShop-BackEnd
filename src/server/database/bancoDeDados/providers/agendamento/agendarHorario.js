...
const reservarHorario = async (reservar,idHorario,role,user_id) => {
    try{
        ...
        if(horarioExist.barbeiro_id !== BarberCliente) {
            return {message:'o horario deve ser o memso do seu barbeiro',status:StatusCodes.CONFLICT}
        }

        if(!horarioExist || horarioExist.status === 'reservado'){
            return {message:'horario não encontrado ou reservado',status:StatusCodes.NOT_FOUND}
        }
