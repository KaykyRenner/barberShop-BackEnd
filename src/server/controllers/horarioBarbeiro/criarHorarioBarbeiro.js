...
const esquemaValidation = yup.object().shape({
    ...
    .of(
        yup.string()
        .matches(/^([01]\d|2[0-3]):([0-5]\d)$/, 'O horário de início deve estar no formato HH:mm.')
        .required()
    )
...
const createHorarioResultados = async (req,res) => {
    try{
        ...
        .first()
        if(!pegandoId) return res.status(StatusCodes.NOT_FOUND).json({ error: 'Barbeiro não encontrado.' });
